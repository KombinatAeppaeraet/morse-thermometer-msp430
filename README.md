# Upcycle a festive IKEA table decoration to tell the temperature with morse code
## Problem
...
In an old project, a MSP430 controller was used to tell the temperature with morse code. Combining this with the *Vintervest J1865*  a festive table thermometer can be created which uses the LED to morse the current temperature. So you have something to talk about and the plastic thing is upcyled. Also you can learn morse code and annoy your relatives at the same time :-) 
...
The extra LED for negative temperature readings from the original project was left out because the thing is used only indoors. The IKEA LED was left in place and is connected to the microcontrollers output via a 120 Ohm resistor. It is really bright to get the readings across.

## Programming on the TI Launchpad

The project uses the msp430g2231 microcontroller which contains an on-chip temerature sensor. The microcontroller is programmed with the *TI launchpad* - this is an Arduino like board, but the microcontroller is socketed. The controller can be removed from the board and transplanted to the IKEA Vinterfest after programming.
...
There is some white plastic on top, acting as reflector for the LED which has to be removed temporary. 
